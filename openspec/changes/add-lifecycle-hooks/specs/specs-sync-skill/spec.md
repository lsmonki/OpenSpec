...
- **THEN** the agent retrieves hook instructions via `openspec instructions --hook pre-sync --change "<name>" --json`
...
- **THEN** the agent retrieves hook instructions via `openspec instructions --hook post-sync --change "<name>" --json`
- **AND** follows each hook instruction in order (schema hooks first, then config hooks)
